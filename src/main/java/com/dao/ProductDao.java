...
public interface ProductDao extends JpaRepository<Product,Integer>, JpaSpecificationExecutor {
 ...
 @Modifying
 @Query("update Product  set name= ?1,price= ?2,type= ?3,brandCode= ?4,size= ?5,sellValue= ?6,marketValue= ?7,color= ?8,induction= ?9 where code= ?10")
 void updateSave(String name, Float price, Long type, Long brandcode, String size, Float sell, Float market, String color, String induction, Long code);
 @Modifying
