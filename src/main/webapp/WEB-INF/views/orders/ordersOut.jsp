		...
		$.ajax({
			...
			success : function(datas) { //function为一个回调函数，data表示为请求servlet后，servlet中返回回来的值
				...
				$.each(datas.list, function(i, obj) {
					...
					if(obj.isOutOfStock==null||obj.isOutOfStock=="0"&&obj.orderState=="1"){
						cont+= '<td><a href="ordersOut?code='+ obj.code+ '&amount='+obj.amount+'&productCode='+obj.productCode+'&usersCode='+obj.usersCode + ' ">确认发货</a></td>';
					}else if(obj.isOutOfStock=="1"){
						cont=cont+'<td>'+"订单已发货"+'</td>'
					}else if(obj.orderState=="0"){
