...
<script type="text/javascript">
	...
	function list(pageNow, pageSize) { //表示页面节点(元素,dom,标签) 加载完成,自动会执行这里面的内容
		$
				.ajax({
					type : "POST",//请求方式为post
					url : "../ssj/productselectStatus?status=1",//请求url
								...
								.each(
										...
										function(i, obj) {
											...
											cont = cont + '<td>' + obj.amount
													+ '</td>';
											cont = cont
													+ '<td><a href="/ssj/productModifyStatus?status=3&code='
													+ obj.code
													+ '">通過</a>　　<a href="/ssj/productModifyStatus?status=2&code='
													+ obj.code
													+ '">不通過</a></td>';
											cont = cont + '</tr>';
										});
