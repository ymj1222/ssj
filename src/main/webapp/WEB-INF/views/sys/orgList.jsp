				...
				$.ajax({
					...
					success : function(datas){ //function为一个回调函数，data表示为请求servlet后，servlet中返回回来的值
						...
						$.each(datas.list,function(i,obj){
							...
							cont = cont +'<td>'+obj.pCode+'</td>';
							if(obj.managerCode!=""&&obj.managerCode!=null){
							cont=cont+'<td> <a href="toDetailsList?code='+obj.managerCode+'">'+obj.managerCode+'</a></td>';
							}else{
								cont=cont+'<td>无</td>';
							}
							if(obj.state=="1"){
								cont+='<td><a href=javascript:del("'+obj.code+'",0); title="删除数据">启用</a>&nbsp;&nbsp;<a href="toOrgUpdate?code='+obj.code+'" title="修改数据">修改</a></td>';
								}
								if(obj.state=="0"){
								cont+='<td><a href=javascript:del("'+obj.code+'",1); title="删除数据">禁用</a>&nbsp;&nbsp;<a href="toOrgUpdate?code='+obj.code+'" title="修改数据">修改</a></td>';
								}	
							cont = cont +'</tr>';							
						});
						...
						<div class="col-md-12">
							...
							<div class="panel">
								<div class="panel-body">
									<i class="lnr lnr-pencil" class="btn btn-primary"><a href="toOrgAdd">添加组织</a></i>
