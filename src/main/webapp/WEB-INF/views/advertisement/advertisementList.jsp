...
<script type="text/javascript">
	...
	function list(pageNow, pageSize) { //表示页面节点(元素,dom,标签) 加载完成,自动会执行这里面的内容
		$.ajax({
			type : "POST",//请求方式为post
			url : "../ssj/advertisementselect",//请求url
			...
			success : function(datas) { //function为一个回调函数，data表示为请求servlet后，servlet中返回回来的值
				...
				$.each(datas.list, function(i, obj) {
					...
					cont = cont + '<td>' + obj.terminalName + '</td>';
					cont = cont + '<td>' + obj.photo+'</td>';
					cont = cont + '<td>' + obj.video + '</td>';
					cont += '<td><a href=javascript:del("' + obj.code
						+ '"); title="删除">删除</a>  ';
					cont += '<a href="advertisementUpdate?code=' + obj.code
						+ '">修改</a></td>';		
					cont = cont + '</tr>';
				});
							...
							<div class="panel">
								<div class="panel-body">
									<div class="navbar-form navbar-left">
										<a href="findadvertisementAdd" class=""><button type="button">广告新增</button></a>
									</div>
									<div class="navbar-form navbar-left">
									...
									<table class="table table-bordered">
										...
										<thead>
											<tr>
												<th width="50">编号</th>
												<th width="50">终端名称</th>
												<th width="100">照片路径</th>
												<th width="100">视频路径</th>
												<th width="50">操作</th>
											</tr>
										</thead>
										<tbody id="tbodyId">
